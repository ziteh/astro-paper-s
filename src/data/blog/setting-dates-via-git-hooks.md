...
while read _ file; do
  ...
  if [ "$draft" = "first" ]; then
    echo "First release of $file, draft set to false and updated removed"
...
For the `sed` to work the frontmatter needs to already have the `updated` key in the frontmatter. There are some other changes you will need to make for the app to build with a blank date, see [further down](#empty-updated-changes)
